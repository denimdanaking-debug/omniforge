...
def test_request_stores_all_core_fields() -> None:
    request = ProviderRequest(
        ...
        stop_sequences=["END"],
        stream=True,
        target_model=ModelId("family", "v1"),
        target_route=RouteId("direct"),
        target_provider=ProviderId("stub"),
    ...
    assert request.target_model == ModelId("family", "v1")
    assert request.target_route == RouteId("direct")
    assert request.target_provider == ProviderId("stub")
...
def test_messages_are_value_objects() -> None:
