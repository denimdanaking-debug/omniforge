...
@pytest.fixture
def adapter(monkeypatch: Any) -> QwenAdapter:
...
async def test_identity(adapter: QwenAdapter) -> None:
    identity = adapter.identity
    assert identity == ProviderIdentity("qwen", "Qwen (Alibaba)", "aliyun.com")
    assert identity.provider_id == "qwen"


async def test_default_model_identity(adapter: QwenAdapter) -> None:
    model = adapter.model_id
    assert model.model_id == "qwen3.8-max"
...
async def test_base_url_override() -> None:
    adapter = QwenAdapter(api_key="test-key", base_url="https://custom.example/v1")
...
async def test_payload_translation_uses_default_model(adapter: QwenAdapter) -> None:
...
async def test_payload_translation_with_overrides(adapter: QwenAdapter) -> None:
    request = ProviderRequest(
        request_id="req-2",
        execution_role=ExecutionRole.CODING,
        risk_level=RiskLevel.R2_NORMAL,
...
async def test_payload_translation_tool_choice_required(adapter: QwenAdapter) -> None:
    request = ProviderRequest(
        request_id="req-3",
        execution_role=ExecutionRole.CODING,
        risk_level=RiskLevel.R2_NORMAL,
...
async def test_response_translation_text(adapter: QwenAdapter) -> None:
    request = ProviderRequest(
        request_id="req-4",
        execution_role=ExecutionRole.CODING,
        risk_level=RiskLevel.R2_NORMAL,
...
async def test_response_translation_tool_calls(adapter: QwenAdapter) -> None:
    request = ProviderRequest(
        request_id="req-5",
        execution_role=ExecutionRole.CODING,
        risk_level=RiskLevel.R2_NORMAL,
...
async def test_response_translation_streaming(adapter: QwenAdapter) -> None:
    request = ProviderRequest(
        request_id="req-6",
        execution_role=ExecutionRole.CODING,
        risk_level=RiskLevel.R2_NORMAL,
