...
def test_orchestration_depends_only_on_normalized_adapter_contract() -> None:
    """Core orchestration modules may import from src.providers.adapter but not adapters."""
    paths = _iter_orchestration_source_paths()
    for path in paths:
        tree = _read_module_ast(path)
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                module = node.module or ""
                if module.startswith("src.providers.") and module not in {
                    ...
                    pytest.fail(
                        ...
                        "only normalized provider contracts are allowed"
                    )
...
def _import_all_orchestration_modules() -> list[Any]:
    ...
    if not _ORCHESTRATION_ROOT.exists():
        return imported
    for _, module_name, _ in pkgutil.iter_modules([str(_ORCHESTRATION_ROOT)], "src.orchestration."):
