...
A ProviderRequest carries everything a provider adapter needs to translate an
OmniForge task into a provider-specific payload, without embedding provider
payload details in the core contract.
"""
...
from providers.contracts.capabilities import ExecutionRole, RiskLevel
...
class ReasoningMode(Enum):
    ...
    Provider adapters translate these into provider-specific controls; unknown
    or unsupported modes must fail eligibility rather than being silently ignored.
    """
...
class ContextPacket:
    ...
    Context packets carry authoritative or derived context without polluting the
    prompt with raw file contents unless intended.
    """
...
class ProviderRequest:
    ...
    target_provider: ProviderId | None = None
