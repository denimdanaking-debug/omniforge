...
class ErrorCategory(Enum):
    ...
    INFRASTRUCTURE = auto()  # Provider-side outage, transport, rate limits
    ROUTE = auto()  # Route/gateway-specific problem
    QUOTA = auto()  # Quota or capacity exhaustion
    AUTH = auto()  # Authentication/authorization
    CAPABILITY = auto()  # Unsupported required capability
    MODEL_QUALITY = auto()  # Bad output from an otherwise healthy model
    PROMPT_CONSTRUCTION = auto()  # System/prompt construction failure
    CANCELLATION = auto()  # Request was cancelled
    UNKNOWN = auto()
...
class ProviderError:
    ...
    Raw provider metadata is captured only when it is safe and useful. Secrets
    and credentials must never be included.
    """
